class PyEnumMixin(object):
    ...
    def _build_cache(cls):
        ...
        for attr in attributes:
            ...
            attr_value = cls.attr_check(attr, getattr(cls, attr))
            id_to_name[attr_value] = attr
            name_to_id[attr] = attr_value

        if verify:
            cls.verify_ids(list(id_to_name.keys()))
    ...
    def num_to_friendly_name(cls):
        name_dict = {}
        for num, name in cls.num_to_name().items():
    ...
    @classmethod
    def get_ids(cls):
        return list(cls.num_to_name().keys())

    @classmethod
    def get_names(cls):
        return list(cls.num_to_name().values())
    ...
    def to_inverted_dict(cls):
        ...
        or not work.
        """
        return {v: k for k, v in cls.to_dict().items()}
