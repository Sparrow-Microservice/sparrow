# Author:Jeeyoung Kim
# Convenient functions for working with iterables.
# before adding stuff here, make sure...
# - it's general enough
# - make sure it's not already in itertools
import itertools
import math
import random
...
def partition(iterable, size):
    ...
    """
    return (
        [snd for fst, snd in grpiter]
        for (grpidx, grpiter) in itertools.groupby(
            enumerate(iterable), lambda idx_elem: idx_elem[0] // size
        )
    )
...
def uniquify(iterable, **kwargs):
    ...
    if set(kwargs.keys()) > set(["key", "sort", "reverse"]):
        raise TypeError("invalid arguments.")
    dictionary = keydict(iterable, key=key)
    # pylint: disable=unnecessary-lambda
    return sorted(iter(dictionary.values()), key=lambda v: sort(v), reverse=reverse)
...
def iterate_multiple_elements(iterator, num_elements):
    ...
    """
    a = iter(iterator)
    for t in zip(*[a for _ in range(num_elements)]):
        yield t
...
def iter_recursive(obj):
    """
    >>> list(iter_recursive(1))
    [1]

    >>> list(iter_recursive([1,2,3]))
    [1, 2, 3]

    >>> list(iter_recursive([1,[2,3],4]))
    [1, 2, 3, 4]
    """
    if not obj:
        return
