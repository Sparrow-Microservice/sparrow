...
def create(project_name, template, port, namespace, project_version):
    ...
    from wish_flask.template import __file__ as template_file
    from wish_flask.utils.os_utils import read_version
    from wish_flask.utils.convert_utils import to_snake_case
...
def gstart(server, bind, timeout, timeout_read, pool_size, auto_reload):
    ...
    """
    from wish_flask.entry.gevent_wsgi import ServerRunner
    ServerRunner(
        server,
        bind=bind,
...
def i18n(action, scan, language, translations, config):
    ...
    """
    from wish_flask.i18n.tool import create_translations, update_translations, compile_translations
    if action == 'create':
...
def micro_client(api_file, package_dir, package_name,
    ...
    from wish_flask.codegen.openapi import OASGen
...
def framework_portal():
    ...
    from wish_flask.framework_portal.command import up
    up()
